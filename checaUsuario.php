...
<body>
    <?php 
        ...
        $usuario = $pdo->query("SELECT * FROM USUARIO WHERE USUARIO_EMAIL = '" . $email . "' AND USUARIO_SENHA = '" . $senha . "'")->fetch();
        $usuarioNome = $pdo->query("SELECT USUARIO_NOME FROM USUARIO  WHERE USUARIO_EMAIL = '" . $email . "'")->fetch();
        ...
        if(count($usuario) == 0){
            ...
            echo "Usuário autenticado, Olá $usuarioNome[0]";
